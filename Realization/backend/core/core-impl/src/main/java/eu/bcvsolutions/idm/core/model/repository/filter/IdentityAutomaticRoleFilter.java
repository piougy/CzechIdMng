	...
	public Predicate getPredicate(Root<IdmIdentity> root, CriteriaQuery<?> query, CriteriaBuilder builder, IdmIdentityFilter filter) {
		...
		subQueryContractRole.where(
				builder.and(
						...
						builder.equal(subRootContractRole.get(IdmIdentityRole_.automaticRole), filter.getAutomaticRoleAttributeId()),
						builder.equal(subRootContractRole.get(IdmIdentityRole_.automaticRole.getName()), "swwfeef")
						));
		subquery.where(
				builder.and(
