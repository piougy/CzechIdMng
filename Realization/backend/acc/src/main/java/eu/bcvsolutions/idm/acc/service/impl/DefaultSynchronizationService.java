...
public class DefaultSynchronizationService implements SynchronizationService {
				...
				public boolean handle(IcSyncDelta delta) {
					...
					try {
						...
						if(itemLog.getSyncActionLog() == null){
							...
							String message = MessageFormat.format("Missing syncActionLog for delta {0}", delta.toString());
							LOG.warn(message);
	...
	public boolean doItemSynchronization(SysSynchronizationConfig config, SysSystem system, SystemEntityType entityType,
			List<SysSchemaAttributeHandling> mappedAttributes, SysSynchronizationLog log, SysSyncItemLog logItem, List<SysSyncActionLog> actionLogs,
			IcSyncDelta delta) {
		...
		if (IcSyncDeltaTypeEnum.CREATE == type || IcSyncDeltaTypeEnum.UPDATE == type
			...
			if (systemEntities.isEmpty()) {
				try {
					...
					if (entity != null) {
						...
						case CREATE_ENTITY:
							 if(true){
							 throw new
							 ProvisioningException(AccResultCode.PROVISIONING_ATTRIBUTE_MORE_UID,
							 "ddddddddddddd");
							 }
					...
					initSyncActionLog(SynchronizationActionType.CREATE_ENTITY, OperationResultType.ERROR, logItem, log, actionLogs);
					LOG.error(message, e);
					throw e;
				}
				...
				try {
					...
					logItem.setMessage(message);
					logItem.addToLog(Throwables.getStackTraceAsString(e));
					initSyncActionLog(SynchronizationActionType.UPDATE_ENTITY, OperationResultType.ERROR, logItem, log, actionLogs);
					LOG.error(message, e);
					throw e;
				}
			} else {
