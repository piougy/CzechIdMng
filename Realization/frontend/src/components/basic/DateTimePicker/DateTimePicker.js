...
import localeCs from 'moment/locale/cs';
import localeEn from 'moment/locale/en-gb';
//
import AbstractFormComponent from '../AbstractFormComponent/AbstractFormComponent';
import Datetime from 'react-datetime';
import LocalizationService from '../../../modules/core/services/LocalizationService';
...
class DateTimePicker extends AbstractFormComponent {
  ...
  setValue(value){
    let dateTime = this._format(value);
    if (this.refs.input){
  ...
  validate(showValidationError){
    ...
    if (this.state.validation){
      ...
      if (result && value) {
        let iso8601Value = moment(value, this.getFormat(), true);
  ...
  _openDialog(){
    let dateTimePicker = this.refs.input;
