...
import eu.bcvsolutions.idm.core.config.web.FlashMessage;
...
public class ProvisioningUpdateProcessor extends AbstractProvisioningProcessor {
	...
	public void processInternal(SysProvisioningOperation provisioningOperation, IcConnectorConfiguration connectorConfig) {	
		...
		//
		this.websocket.convertAndSendToUser(securityService.getUsername(), "/queue/messages", new FlashMessage("Provisioning účtu [" + provisioningOperation.getSystemEntityUid() + "] na systém [" + provisioningOperation.getSystem().getName() + "] úspěšně proběhl."));
	}
}
