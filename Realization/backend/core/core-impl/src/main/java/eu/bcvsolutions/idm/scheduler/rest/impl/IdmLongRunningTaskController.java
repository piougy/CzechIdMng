...
import eu.bcvsolutions.idm.scheduler.service.impl.TestTaskExecutor;
...
public class IdmLongRunningTaskController extends AbstractReadWriteEntityController<IdmLongRunningTask, LongRunningTaskFilter> {
	...
	@ResponseBody
	@RequestMapping(method = RequestMethod.PUT, value = "/test/create")
	public ResponseEntity<?> test() {
		TestTaskExecutor taskExecutor = new TestTaskExecutor();
		taskExecutor.setCount(600L);
		entityLookupService.getEntityService(IdmLongRunningTask.class, IdmLongRunningTaskService.class).execute(taskExecutor);
		//
		return new ResponseEntity<Object>(HttpStatus.NO_CONTENT);
	}
}
