...
public class DefaultIdmRoleRequestService
	...
	private IdmRoleRequestDto executeRequestInternal(EntityEvent<IdmRoleRequestDto> requestEvent) {
		...
		IdmIdentityRoleFilter automaticRoleFilter = new IdmIdentityRoleFilter();
		automaticRoleFilter.setIdentityId(identity.getId());
		automaticRoleFilter.setAutomaticRole(Boolean.TRUE);
		List<IdmIdentityRoleDto> automaticRoles = identityRoleService.find(automaticRoleFilter, null).getContent();
		...
		concepts.stream().filter(concept -> {
			...
			if (!cancelInvalidConcept(automaticRoles, concept, request)) {
				// assign new role
