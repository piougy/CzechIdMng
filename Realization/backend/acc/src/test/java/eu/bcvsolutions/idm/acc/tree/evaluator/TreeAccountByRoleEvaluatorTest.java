...
/**
 * Test for TreeAccountByRoleEvaluatorTest
 *
 * @author Kučera
 *
 */
public class TreeAccountByRoleEvaluatorTest extends AbstractIntegrationTest {

	@Autowired private TestHelper helper;
	@Autowired private IdmRoleService treeNodeService;
	...
	public void testRoleWithoutEvaluator() {
		...
		try {
			...
			IdmRole role = treeNodeService.get(TEST_ROLE_ID, IdmBasePermission.READ);

			assertEquals(TEST_ROLE_ID, role.getId());
			assertEquals(1, treeNodeService.findSecured(null, null, IdmBasePermission.READ).getTotalElements());
	...
	private IdmIdentityDto createIdentityWithRole(boolean transitive) {
		...
		if (transitive) {
			...
			readRoleAccountPolicy.setGroupPermission(AccGroupPermission.TREEACCOUNT.getName());
			readRoleAccountPolicy.setAuthorizableType(AccTreeAccount.class.getCanonicalName());
			readRoleAccountPolicy.setEvaluator(TreeAccountByRoleEvaluator.class);
