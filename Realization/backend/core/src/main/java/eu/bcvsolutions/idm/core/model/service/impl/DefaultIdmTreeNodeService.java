...
import eu.bcvsolutions.idm.core.exception.DefaultTreeNodeErrorModel;
...
public class DefaultIdmTreeNodeService extends AbstractReadWriteEntityService<IdmTreeNode, TreeNodeFilter> implements IdmTreeNodeService {
	...
	public void delete(IdmTreeNode entity) {
		...
		if (!listWorkingPosition.isEmpty()) {
			throw new DefaultTreeNodeErrorModel(CoreResultCode.TREE_NODE_CANNOT_DELETE,  ImmutableMap.of("node", entity.getName()));
		}
	...
	private void testNode(IdmTreeNode node) {
		if (checkParents(node)) {
			throw new DefaultTreeNodeErrorModel(CoreResultCode.TREE_NODE_BAD_PARRENT,  "TreeNode ["+node.getName() +"] have bad parent.");
		}
		...
		if (checkCorrectType(node)) {
			throw new DefaultTreeNodeErrorModel(CoreResultCode.TREE_NODE_BAD_PARRENT,  "TreeNode ["+node.getName() +"] have bad type.");
		}
	}
