        ...
        .then((error) => {
          if (successEntities.length > 0) {
            dispatch(this.flashMessagesManager.addMessage({
              level: 'success',
              message: this.i18n(`action.${operation}.success`, {
          ...
          if (failedEntities.length > 0) {
            dispatch(this.flashMessagesManager.addMessage({
              level: 'info',
              message: this.i18n(`action.${operation}.failed`, {
                count: failedEntities.length,
                records: this.getNiceLabels(failedEntities).join(', '),
                record: this.getNiceLabel(failedEntities[0])
              })
            }));
          }
