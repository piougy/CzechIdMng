...
public class IdmPasswordController extends AbstractReadWriteDtoController<IdmPasswordDto, IdmPasswordFilter> {
	...
	private final IdmPasswordService service;
	...
	@ResponseBody
	@PreAuthorize("hasAuthority('" + CoreGroupPermission.PASSWORD_READ + "')")
	@RequestMapping(value = "/search/identity/{identityId}", method = RequestMethod.GET)
	@ApiOperation(
			value = "Search password by identity", 
			nickname = "searchPasswordByIdentity",
			response = IdmPasswordDto.class, 
			tags = { IdmPasswordController.TAG }, 
			authorizations = {
				@Authorization(value = SwaggerConfig.AUTHENTICATION_BASIC, scopes = { 
	...
	public ResponseEntity<?> findByIdentity(@PathVariable @NotNull String identityId) {
		BaseDto identityDto = lookupService.lookupDto(IdmIdentityDto.class, identityId);
		if (identityDto == null) {
			throw new EntityNotFoundException(getService().getEntityClass(), identityId);
		}
		IdmPasswordDto passwordDto = service.findOneByIdentity(UUID.fromString(identityDto.getId().toString()));
		if (passwordDto == null) {
			throw new EntityNotFoundException(getService().getEntityClass(), identityId);
		}
		return new ResponseEntity<>(toResource(passwordDto), HttpStatus.OK);
	}
