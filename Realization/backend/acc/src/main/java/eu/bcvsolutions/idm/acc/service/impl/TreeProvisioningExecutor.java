...
public class TreeProvisioningExecutor extends AbstractProvisioningExecutor<IdmTreeNode> {
	...
	public void changePassword(IdmTreeNode node, PasswordChangeDto passwordChange) {
		...
		List<? extends EntityAccountDto> treeAccountList = treeAccountService.find(filter, null).getContent();
	...
	public List<AttributeMapping> resolveMappedAttributes(String uid, AccAccount account, IdmTreeNode entity, SysSystem system, SystemEntityType entityType) {
		...
		List<? extends EntityAccountDto> entityAccoutnList = treeAccountService.find(filter, null).getContent();
