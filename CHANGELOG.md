...
- 🟡 (_edited 15.1.2021_) [#2355](https://redmine.czechidm.com/issues/2355) - For every new or changed item in confidential storage will be generated new unique initial vector. For security reasons, you should generate new initial vector for old values in the confidential storage, because they reused the same initial vector. Also a warning message would appear in the application log, until you resave confidential storage values with the unique initial vector. Use the long running task [``Change confidential storage key (ChangeConfidentialStorageKeyTaskExecutor)``](https://wiki.czechidm.com/devel/documentation/security/dev/confidential-storage#change_confidential_storage_key), where you put your current key as ``old key``. (Your current confidential storage key is saved in the file specified by the property ``cipher.crypt.secret.keyPath`` or directly in the property ``cipher.crypt.secret.key``, both  would be in the ``application-xxx.properties`` file on the server.)
- 🟡 (_edited 15.1.2021_) [#2355](https://redmine.czechidm.com/issues/2355) - Check if your application server is starting with the parameter ``java.security.egd=/dev/./urandom``. If not, add it into starting parameter for the application server (typically ``/etc/systemd/system/tomcat.service`` in Linux, Java Options set by ``Tomcat8w.exe`` in Windows).
